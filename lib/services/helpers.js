...
export const isCustomView = (view, userSlug) => {
	return view.slug.startsWith('view-user-created-view') && view.markers.length === 1 && view.markers[0] === userSlug
}
...
const getTypesFromSchema = (schema) => {
	const types = _.get(schema, [ 'properties', 'type', 'const' ]) ||
...
export const formatTimestamp = (stamp, prefix = false) => {
 ...
 */
export const getMergedSchema = (...typeCards) => {
	const schemas = _.map(typeCards || {}, (typeCard) => {
		return _.get(typeCard, [ 'data', 'schema' ], {})
	})
	return clone(skhema.merge(schemas))
}
...
const isStringItem = (item) => {
	return item.type === 'string' || (_.isArray(item.type) && _.includes(item.type, 'string'))
}

const isArrayOfStringsItem = (item) => {
	return item.type === 'array' && item.items && isStringItem(item.items)
}

const isFullTextSearchableField = (item) => {
	return isStringItem(item) || isArrayOfStringsItem(item)
}

const getItemSearchQuery = (term, fullTextSearch) => {
...
const getSearchableFieldQuerySchema = (item, term) => {
 ...
 */
export const createFullTextSearchFilter = (schema, term, options = {}) => {
...
export const getUserTooltipText = (user, options = {}) => {
	...
	return _.compact([
		options.hideName ? null : _.truncate(fullName, 30),
		options.hideEmail ? null : _.truncate(email, 30),
		options.hideUsername ? null : _.truncate(slug.slice(5), 30)
	]).join('\n')
}
 ...
 */
export const checkFieldAgainstOmissions = (schema, fieldName, omissions) => {
 ...
 */
export const getPathsInSchema = (schema, omissions, pathToField = []) => {
	let newSchema = []
	...
	for (const field of topLevelFields) {
		...
		if (fieldSchema.type === 'object') {
			newSchema = _.concat(newSchema, getPathsInSchema(fieldSchema, omissions, updatedPathToField))
		} else {
			const matchesAnOmission = checkFieldAgainstOmissions(fieldSchema, field, omissions)
...
export const getLinkedCardInfo = (link, card) => {
...
export const formatCreatedAt = (card) => {
...
const jellyfishUrlRegex = new RegExp(`^https?://(www.)?${window.location.host}`)
