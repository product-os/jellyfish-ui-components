...
ava('.createFullTextSearchFilter() works on arrays of strings with the \'fullTextSearch\' field set on the array', (test) => {
	const searchTerm = 'test'
	const schema = {
		properties: {
			folders: {
	...
	const filter = helpers.createFullTextSearchFilter(schema, searchTerm)
	test.is(filter.anyOf[0].properties.folders.contains.fullTextSearch.term, searchTerm)
})

ava('.createFullTextSearchFilter() works on arrays of strings', (test) => {
	const searchTerm = 'test'
	const schema = {
		properties: {
			folders: {
	...
	const filter = helpers.createFullTextSearchFilter(schema, searchTerm)
	test.is(filter.anyOf[0].properties.folders.contains.regexp.pattern, searchTerm)
})
